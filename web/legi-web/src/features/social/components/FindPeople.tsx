      ...
      <div className="p-4">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-lg border border-stone-300 py-1.5 pl-9 pr-3 text-sm focus:outline-none focus:ring-1 focus:ring-green-600"
          />
        </div>

        <div className="mt-3 min-h-10">
          {isTooShort ? (
            <p className="text-xs text-stone-400">{t("feed.findPeopleMinChars")}</p>
          ) : isInvalid ? (
            <p className="text-xs text-red-600">{t("feed.findPeopleInvalid")}</p>
          ) : usersQuery.isLoading ? (
            <p className="text-xs text-stone-400">{t("feed.findPeopleLoading")}</p>
          ) : usersQuery.isError ? (
            <div className="space-y-2">
              <p className="text-xs text-stone-500">{t("feed.findPeopleError")}</p>
              <Button
                variant="outline"
                size="sm"
                onClick={() => {
            ...
            <p className="text-xs text-stone-400">{t("feed.findPeopleEmpty")}</p>
          ) : users.length > 0 ? (
            ...
            <p className="text-xs text-stone-400">{t("feed.findPeopleHint")}</p>
          )}
        </div>
      </div>
    </Card>
